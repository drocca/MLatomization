...
loop0=0
scale=numpy.zeros(276)
while loop0 < 276:
    xtmp=[float(classarray[i].coulomb[loop0]) for i in range(7102) ]
    scale[loop0]=max(xtmp)
    loop0=loop0+1

#print [classarray[i].coulomb[loop0] for i in range(7102) ] 
print scale

Xm=numpy.zeros((900,276))
yyy=numpy.zeros((900))
...
while loop0 < 7102:
    if (classarray[loop0].logic==1):
        ...
        Xm[m1][:]=numpy.divide(classarray[loop0].coulomb,1.0)  
        m1=m1+1
    loop0=loop0+1

#Xtmp=
#Xm_scaled=MaxAbsScaler().fit_transform(Xm)
#Xm=MaxAbsScaler().inverse_transform(Xm_scaled)
#Xm=Xm_scaled
#print Xm[:, 273]
#print Xm[:, 273]/scale[273]
#print Xm[899][:]

Xho=numpy.zeros((100,276))

loop0=0
m1=0
while loop0 < 7102:
    if (classarray[loop0].logic==2):
        Xho[m1][:]=numpy.divide(classarray[loop0].coulomb,1.0)
        m1=m1+1
    loop0=loop0+1

#Xho_scaled = MaxAbsScaler().transform(Xho)
#Xho=Xho_scaled

#krr = KernelRidge(gamma=.00000762939453,kernel='rbf',alpha=0.000007629394) # Gaussian kernel
krr = KernelRidge(gamma=.0002441,kernel='laplacian',alpha=0.00000000093132257)
#krr = svm.SVR(kernel='laplacian',C=1073741829.32,gamma=.0002441)
#krr = svm.SVR(kernel='rbf',gamma=.00000762939453,C=131072)
#krr = MLPRegressor(solver='lbfgs', activation='relu', tol=1e-4, alpha=5e-5, hidden_layer_sizes=(276,276), max_iter=4000, random_state=1)
#print get
#print kernel.shape
krr.fit(Xm, yyy)
prediction=krr.predict(Xho)
